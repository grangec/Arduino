...
//=================================================================================================================
boolean readTeleInfo(boolean emul)
{
...
#define maxFrameLen 280
  ...
  //String emuTeleInfo[] = {"ADCO 0xxxxxxxxxx3 *","OPTARIF HC.. <","ISOUSC 30 9","HCHC 014460852 $","HCHP 012506372 -","PTEC HP..  ","IINST 002 Y","IMAX 035 G","PAPP 00520 (","HHPHC C .","MOTDETAT 000000 B"};
...
/* autre exemple
 *  "ADCO 2xxxxxxxxxx9 2","ADCO 2xxxxxxxxxx9 B",
 *  
ADCO 0xxxxxxxxxx5 D
OPTARIF BASE 0
ISOUSC 30 9
...
*/
  ...
  while (charIn != startFrame and not emul)
  ...
  { // tant que des octets sont disponibles en lecture : on lit les caractères
    // if (Serial.available())
    if (mySerial->available() or emul)
    {
      if(emul) {
        bufferLen=emuTeleInfo[sequenceNumber].length()+1;
        emuTeleInfo[sequenceNumber].toCharArray(bufferTeleinfo, bufferLen);
        charIn=endLine;
        
        dbg("Emul. Seq. :'");
        dbg(String(sequenceNumber));
        dbgln("'");
        
        dbg("emuTeleInfo :'");
        dbg(emuTeleInfo[sequenceNumber]);
        dbgln("'");
        
        dbg("bufferLen :'");
        dbg(String(bufferLen,DEC));
        dbgln("'");
      ...
      if (charIn == endLine)
      {
        checkSum = bufferTeleinfo[bufferLen-2];
        dbg("CheckSum ");
        dbgln(String(bufferTeleinfo[bufferLen-2]));
        if (chksum(bufferTeleinfo, bufferLen) == checkSum)
        { // we clear the 1st character
          strncpy(&bufferTeleinfo[0], &bufferTeleinfo[0], bufferLen - 2);
...
void loop() {
  dbgln("=>Début Loop Debug");
  
  teleInfoReceived = readTeleInfo(testOregon);
  
  if (teleInfoReceived)
  {
    dbgln("teleInfoReceived !");
