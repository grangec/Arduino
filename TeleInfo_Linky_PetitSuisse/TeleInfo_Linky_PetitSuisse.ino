...
const unsigned long TELEINFO_BAUDS = 1200;
...
//boolean testOregon = true;
boolean testOregon = false;// Debug Transmission 433Mhz
boolean debug = true;      // Debug global
boolean debugTI = true;    // debug TeleInfo
...
void TeleInfo(String version)
{
  ...
  mySerial = new SoftwareSerial(TELEINFO_PIN, TELEINFO_PIN+1); // RX, TX
  pinMode(TELEINFO_PIN,INPUT);
  pinMode(TELEINFO_PIN+1,OUTPUT);
//  mySerial->begin(1200);
  mySerial->begin(TELEINFO_BAUDS);
  pgmVersion = version;
...
/*  if (debugTI) {
    if (mySerial->isListening()) {
      ...
      Serial.println("Port One isn't listening!");    
    }
  }
 */
...
*/
  ...
  OPTARIF = "HC..";
  ISOUSC = 20;
  HCHC = 004063750; // compteur Heures Creuses en W
  HCHP = 004423070; // compteur Heures Pleines en W
  PTEC = "HP.."; // Régime actuel : HPJB, HCJB, HPJW, HCJW, HPJR, HCJR
  HHPHC = 'E';
  IINST1 = 000; // intensité instantanée en A pour la phase 1
  IINST2 = 000; // intensité instantanée en A pour la phase 2
  IINST3 = 000; // intensité instantanée en A pour la phase 3
  IMAX1 = 006; // intensité maxi pour la phase 1 en A
  IMAX2 = 015; // intensité maxi pour la phase 2 en A
  IMAX3 = 015; // intensité maxi pour la phase 3 en A
  PMAX = 05710; // Puissance maximale triphasee atteinte (en W)
  PAPP = 10; // puissance apparente en VA
  MOTDETAT = "000000";
...
#define maxFrameLen 280
  ...
  int bufferLen = 0;
  int checkSum;
  unsigned int cptNAvail = 0;

  //String emuTeleInfo[] = {"ADCO 70060936XXXX V","OPTARIF HC.. <","ISOUSC 20 8","HCHC 004063750 _","HCHP 004423070 '","PTEC HP..  ","IINST1 001 I","IINST2 000 I","IINST3 001 K","IMAX1 006 6","IMAX2 016 8","IMAX3 015 8","PMAX 05710 3","PAPP 00280 +","HHPHC E 0","MOTDETAT 000000 B","PPOT 00 #"};
  ...
  //if(debugTI) Serial.println(F("readTeleinfo StartFrame"));

  while (mySerial->isListening() and charIn != startFrame and !testOregon)
  { // "Start Text" STX (002 h) is the beginning of the frame
    //if (mySerial->available()) {
      if(debugTI) Serial.println(F("mySerial Available"));
      charIn = mySerial->read();
      if(debugTI) Serial.print(F("charIn : "));
      if(debugTI) Serial.println(charIn,HEX);
      charIn = charIn & 0x7F; // Serial.read() vide buffer au fur et à mesure
      if(debugTI) Serial.print(F("charIn & 0x7F :"));
      if(debugTI) Serial.println(charIn,HEX);
    /*}else
     {
       if(debugTI) 
       {
         if ((cptNAvail % 8196) == 0) {
         Serial.print(F("NO startFrame , cpt : "));
         Serial.println(cptNAvail,DEC);
         }
         cptNAvail++;
       }
     }*/
  ...
  if(debugTI) Serial.println(F("readTeleinfo EndFrame"));
  while (charIn != endFrame and comptChar<=maxFrameLen and !testOregon)
  { // tant que des octets sont disponibles en lecture : on lit les caractères
    ...
    if (mySerial->available())
    {
      ...
      if(debugTI) Serial.print(F("cptChar/charIn & 0x7F :"));
      if(debugTI) Serial.print(comptChar,DEC);
      if(debugTI) Serial.print(F("/"));
      if(debugTI) Serial.println(charIn,HEX);
      // incremente le compteur de caractère reçus
      comptChar++;
      if (charIn == startLine)
        bufferLen = 0;
  ...
  if(debug) Serial.println(F("readTeleinfo RETURN"));
  return true;
}
...
void loop() {
  if(debug) Serial.println(F("LOOP DEB"));
  //Serial.println(ADCO);
